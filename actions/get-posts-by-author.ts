...
export async function getPostsByAuthor(
  username: string,
  cursor?: string
): Promise<GetPostsResult & { authorName: string | null }> {
  try {
    ...
    if (!author)
      return { posts: [], nextCursor: null, hasMore: false, authorName: null }
    ...
    return { posts: trimmed, nextCursor, hasMore, authorName: author.name }
  } catch {
    return {
      posts: [],
      nextCursor: null,
